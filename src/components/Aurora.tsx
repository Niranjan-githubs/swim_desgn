...
const Aurora: React.FC<AuroraProps> = ({
  colorStops = ['#00D4FF', '#FFFFFF', '#00D4FF'], // Pool-themed blues with white
  amplitude = 1.0,
  blend = 0.5,
  speed = 1.0,
  intensity = 1.0,
  ...
  const initAurora = useCallback(() => {
    ...
    cleanup(); // Clean up any existing renderer
    ...
    const update = (t: number) => {
      ...
      if (programRef.current && rendererRef.current) {
        ...
        const updatedColorStops = colorStops.map((hex: string) => {
          const c = new Color(hex);
          return [c.r, c.g, c.b];
        });
        programRef.current.uniforms.uColorStops.value = updatedColorStops;
  ...
  }, [colorStops, amplitude, blend, speed, intensity, cleanup]);
